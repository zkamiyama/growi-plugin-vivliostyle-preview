...
function ensureIframe(): HTMLIFrameElement {
  ...
  const controlScript = `/* vivliostyle control script (patched for fallback + hash params) */\n(function(){\n  const status=document.getElementById('vs-status');\n  const logEl=document.getElementById('log');\n  const fallbackEl=document.getElementById('fallbackHtml');\n  function setStatus(s){ try{ console.log('[vivlio:status]', s); }catch{} }\n  function log(m){ try{ if(logEl){ logEl.style.display='block'; logEl.textContent += m+'\\n'; if(logEl.textContent.length>16000) logEl.textContent = logEl.textContent.slice(-14000);} }catch{} }\n  try { const origErr=console.error; console.error=function(){ try{ log('[iframe.console.error] '+Array.from(arguments).map(a=>String(a)).join(' ')); }catch{}; try{ origErr.apply(console, arguments); }catch{} }; window.addEventListener('error', ev=>{ try{ log('[iframe.onerror] '+ev.message+' @'+ev.filename+':'+ev.lineno); }catch{} }); window.addEventListener('unhandledrejection', ev=>{ try{ log('[iframe.unhandledrejection] '+ev.reason); }catch{} }); }catch{}\n  let pending=null; let ready=false; let lastUrl=null;\n  function makeBlobUrl(d){ try{ const htmlDoc = d.full || '<!DOCTYPE html><html><head><meta charset=\\"utf-8\\"></head><body>'+(d.html||'')+'</body></html>'; if(lastUrl) try{ URL.revokeObjectURL(lastUrl); }catch{}; const blob=new Blob([htmlDoc],{type:'text/html'}); lastUrl=URL.createObjectURL(blob); return {htmlDoc, url:lastUrl}; }catch(e){ log('makeBlobUrl fail '+e); return null; } }\n  function heuristicReady(){ try { const st=document.body.getAttribute('data-vivliostyle-viewer-status'); const pc=document.querySelectorAll('[data-vivliostyle-page-container]').length; return !!st || pc>0; } catch { return false; } }\n  function pageCount(){ try{ return document.querySelectorAll('[data-vivliostyle-page-container]').length; }catch{return 0;} }\n  window.addEventListener('message',e=>{ const d=e.data; if(!d||d.type!=='HTML_FULL') return; if(!pending) pending=d; if(!ready){ if(d.progressive){ try{ fallback(d.full || d.html || ''); setStatus('progressive'); }catch(err){ log('progressive fail '+err); } } const mk = makeBlobUrl(d); if(mk){ /* bookMode=true 付与でページ組版を強制 */ location.hash='#src='+encodeURIComponent(mk.url)+'&bookMode=true'; log('early hash set len='+mk.htmlDoc.length); } else { log('mk null'); } } else { log('update ignored (ready single-load mode)'); } });\n  function extractBody(htmlDoc){ try{ const m=htmlDoc.match(/<body[^>]*>([\s\S]*?)<\/body>/i); if(m) return m[1]; return htmlDoc; }catch{return htmlDoc;} }\n  function fallback(htmlDoc){ try{ if(!fallbackEl) return; const bodyInner=extractBody(htmlDoc); fallbackEl.innerHTML=bodyInner; fallbackEl.style.display='block'; fallbackEl.style.zIndex='1000'; fallbackEl.style.background='#fff'; setStatus('fallback'); }catch(err){ log('fallback err '+err); } }\n  function poll(){ let c=0; const iv=setInterval(()=>{ c++; if(c%10===0) log('poll attempt '+c); if(heuristicReady()){ clearInterval(iv); ready=true; setStatus('ready'); const pc=pageCount(); log('heuristic ready pages='+pc); if(pc>0){ try{ fallbackEl && (fallbackEl.style.display='none'); }catch{} parent.postMessage({type:'VIVLIO_RENDER_DONE', pages: pc},'*'); } else { /* ページ無い=viewer 未組版。フォールバック継続表示 */ parent.postMessage({type:'VIVLIO_RENDER_DONE', note:'noPagesYet'},'*'); } return; } if(c===30){ /* 早期に viewer が何も表示しない場合 viewport を半透明化 */ try{ const vp=document.getElementById('vivliostyle-viewer-viewport'); if(vp) vp.style.opacity='0.05'; }catch{} } if(c>200){ clearInterval(iv); setStatus('timeout'); log('viewer timeout heuristic'); /* タイムアウト時は常に fallback 表示維持 */ parent.postMessage({type:'VIVLIO_RENDER_DONE', error:'timeout'},'*'); } },100); }\n  function injectViewer(){ setStatus('inject'); let raw; try{ raw = atob('${viewerB64}'); }catch(e){ setStatus('b64-decode-error'); log('b64 decode error '+e); parent.postMessage({type:'VIVLIO_RENDER_DONE', error:'b64 '+e},'*'); return; } const blob=new Blob([raw],{type:'text/javascript'}); const u=URL.createObjectURL(blob); const s=document.createElement('script'); s.src=u; s.onload=()=>{ URL.revokeObjectURL(u); setStatus('script'); poll(); }; s.onerror=e=>{ setStatus('err'); log('script err'); parent.postMessage({type:'VIVLIO_RENDER_DONE', error:'script load'},'*'); }; document.head.appendChild(s);}\n  try { injectViewer(); } catch(er){ log('injectViewer throw '+er); parent.postMessage({type:'VIVLIO_RENDER_DONE', error:'inject '+er},'*'); }\n})();\n`;
  // base64 化（UTF-8）

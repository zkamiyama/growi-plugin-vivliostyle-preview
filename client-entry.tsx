...
function ensureIframe(): HTMLIFrameElement {
  ...
  const control = `/* vivlio control (improved fallback) */\n(function(){\n  const log=(m)=>{ try{console.log('[vivlio:host]',m);}catch{} };\n  let lastBlob=null; let pending=null; let booted=false;\n  // Ensure hash param exists early to avoid internal errors
  try { if(!location.hash.includes('src=')){ const empty=URL.createObjectURL(new Blob(['<!DOCTYPE html><html><head><meta charset=\\"utf-8\\"></head><body></body></html>'],{type:'text/html'})); location.hash='#src='+encodeURIComponent(empty)+'&bookMode=true'; } } catch{}
  function mk(full){ if(lastBlob) try{URL.revokeObjectURL(lastBlob);}catch{}; const b=new Blob([full],{type:'text/html'}); lastBlob=URL.createObjectURL(b); return lastBlob; }\n  function build(d){ return d.full||'<!DOCTYPE html><html><head><meta charset=\\"utf-8\\"></head><body>'+(d.html||'')+'</body></html>'; }\n  function showFallback(full){ try{ const m=full.match(/<body[^>]*>([\\s\\S]*?)<\\/body>/i); const body=m?m[1]:full; const fb=document.getElementById('fallbackHtml'); if(fb){ if(!fb.innerHTML) fb.innerHTML=body; fb.style.display='block'; } }catch(e){ log('fallback err '+e); } }\n  function apply(d){ const f=build(d); const u=mk(f); location.hash='#src='+encodeURIComponent(u)+'&bookMode=true'; showFallback(f); log('hash set len='+f.length); }

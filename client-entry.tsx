...
function ensureIframe(): HTMLIFrameElement {
  ...
  window.addEventListener('message',e=>{ const d=e.data; if(!d||d.type!=='HTML_FULL') return; if(!ready){
      ...
      pending=d; log('queue msg htmlLen='+(d.full?d.full.length:(d.html?d.html.length:0))); log('html head preview:'+(d.full||d.html||'').slice(0,160).replace(/\s+/g,' '));
    } else { render(d); } });
  function render(d){ try{ const htmlDoc = d.full || '<!DOCTYPE html><html><head><meta charset="utf-8"></head><body>'+(d.html||'')+'</body></html>'; log('render start htmlLen='+htmlDoc.length); if(lastUrl) try{URL.revokeObjectURL(lastUrl);}catch{}; const blob=new Blob([htmlDoc],{type:'text/html'}); const url=URL.createObjectURL(blob); lastUrl=url; location.hash='#src='+encodeURIComponent(url); try{ if(window.VivliostyleViewer&&window.VivliostyleViewer.viewer&&window.VivliostyleViewer.viewer.load){ const vv=window.VivliostyleViewer.viewer; log('call viewer.load('+url+')'); let ret; try{ ret=vv.load(url); }catch(er){ log('direct load throw '+er); }
        if(ret && typeof ret.then==='function'){ ret.then(()=>{ log('viewer.load resolved pages='+(vv.pageManager?.pages?.length)); }).catch(e=>{ log('viewer.load rejected '+e); }); }
      } else { log('viewer.load not available'); } }catch(ex){ log('viewer.load fail '+ex); }
      setStatus(pending?'render+flush':'render'); try{ if(fallbackEl){ fallbackEl.style.transition='opacity .25s'; fallbackEl.style.opacity='0'; setTimeout(()=>{ fallbackEl.style.display='none'; },300);} }catch{}
      setTimeout(diagPages,500);
      setTimeout(()=>{ diagPages(); if(!hasPages()){ log('no pages after 1500ms -> restore fallback'); try{ fallbackEl.style.display='block'; fallbackEl.style.opacity='1'; setStatus('fallback-restore'); }catch{} } },1500);
      setTimeout(diagPages,3000);
    }catch(ex){ log('render err '+ex); fallback(d.full || d.html || ''); }}

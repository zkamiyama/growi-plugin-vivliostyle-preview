...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  React.useEffect(() => {
    ...
    const handleNavigation = (previousHref: string, nextHref: string) => {
      if (disposed) return;
      dbg('[VivlioDBG][AppContext] page change detected', { previousHref, nextHref, contextId: contextIdRef.current });
      setIsVivliostyleActive((prev) => (prev ? false : prev));
      setForced((prev) => (prev !== null ? null : prev));
      try { clearVivlioCssCache(); } catch (error) { /* ignore */ }
      try {
        const detail = { previousHref, nextHref, contextId: contextIdRef.current };
        window.dispatchEvent(new CustomEvent('vivlio:page-changed', { detail }));
      } catch (error) { /* ignore */ }
    };
    ...
    const check = () => {
      ...
      if (currentHref !== lastHref) {
        ...
        handleNavigation(prevHref, currentHref);
      }
    };
    ...
    return () => {
      ...
      window.removeEventListener('popstate', onNavigationEvent);
      window.removeEventListener('hashchange', onNavigationEvent);
    };
  }, []);

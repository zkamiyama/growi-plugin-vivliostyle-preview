...
self.addEventListener('message', async (ev: MessageEvent) => {
  ...
  const metadata = data && data.metadata ? data.metadata : undefined;
  const postProgress = (stage: string) => {
    try {
      (self as any).postMessage({ type: 'progress', seq, stage, timestamp: Date.now() });
    } catch (e) { /* ignore */ }
  };
  postProgress('worker:init');
  ...
  postProgress('worker:normalize:done');
  if (options && options.enableBreaks) {
          postProgress('worker:hardBreaks:start');
          try {
            md = applyHardBreaks(md);
          } catch (hbErr) {
            console.warn('[vfmWorker] applyHardBreaks failed, continuing without it', hbErr);
          }
          postProgress('worker:hardBreaks:done');
        }
        if (typeof metadata !== 'undefined') {
          postProgress('worker:stringify:start');
          if (typeof options !== 'undefined') {
            html = stringify(md, options, metadata);
          } else {
            html = stringify(md, undefined, metadata);
          }
        } else {
          postProgress('worker:stringify:start');
          html = typeof options !== 'undefined' ? stringify(md, options) : stringify(md);
        }
        postProgress('worker:stringify:done');
      } catch (e) {
    ...
    // respond with structured object to avoid main-thread JSON.parse cost
    try {
      postProgress('worker:payload:ready');
      (self as any).postMessage({ seq, ok: true, html });
    } catch (e) {
      // fallback: some older environments may expect strings
      (self as any).postMessage(JSON.stringify({ seq, ok: true, html }));
    }
  } catch (e) {
    try {
      (self as any).postMessage({ seq: null, ok: false, error: String(e) });
    } catch (ee) {

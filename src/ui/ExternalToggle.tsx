...
export const ExternalToggle: React.FC = () => {
  ...
  React.useEffect(() => {
    ...
    if (!initialAnchor) {
      ...
      console.debug('[VivlioDBG][ExternalToggle] no initial anchor found');
      return; // アンカーが見つからない場合は何もしない
    }

    // eslint-disable-next-line no-console
    console.debug('[VivlioDBG][ExternalToggle] initial anchor found', { isEditing, resolved: resolvedRef.current, anchorClass: initialAnchor.className });
    ...
    if (resolvedRef.current) {
      const existingWrapper = initialAnchor.parentElement?.querySelector('.vivlio-inline-toggle') as HTMLElement | null;
      if (existingWrapper) {
        // eslint-disable-next-line no-console
        console.debug('[VivlioDBG][ExternalToggle] wrapper already exists, reusing');
        setWrapperEl(existingWrapper);
        return;
      } else {
        // eslint-disable-next-line no-console
        console.debug('[VivlioDBG][ExternalToggle] resolved but wrapper missing, resetting');
        resolvedRef.current = false;
      }
    }
    ...
    if (!resolvedRef.current) {
      if (!initialAnchor.parentElement) {
        // eslint-disable-next-line no-console
        console.debug('[VivlioDBG][ExternalToggle] initialAnchor has no parentElement');
        return;
      }
    ...
    const checkAndAttach = () => {
      // eslint-disable-next-line no-console
      console.debug('[VivlioDBG][ExternalToggle] checkAndAttach called', { isEditing, resolved: resolvedRef.current });

      if (isEditing) {
        if (!resolvedRef.current) {
          const immediate = findAnchorOnce();
          if (immediate) {
            // アンカーが見つかった場合、ボタン作成処理をここで実行
            // eslint-disable-next-line no-console
            console.debug('[VivlioDBG][ExternalToggle] found anchor in checkAndAttach, creating button');

            if (immediate.parentElement) {
              let wrapper = immediate.parentElement.querySelector('.vivlio-inline-toggle') as HTMLElement | null;
              if (!wrapper) {
                wrapper = document.createElement('span');
                wrapper.className = 'vivlio-inline-toggle';
                wrapper.style.marginLeft = '6px';
                immediate.parentElement.insertBefore(wrapper, immediate.nextSibling);
                // eslint-disable-next-line no-console
                console.debug('[VivlioDBG][ExternalToggle] wrapper created in checkAndAttach');

                // ボタン作成処理の簡易版を実行
                setWrapperEl(wrapper);
                resolvedRef.current = true;
                primaryAnchorRef.current = immediate;
                setAnchorClasses(immediate.className);
              } else {
                // eslint-disable-next-line no-console
                console.debug('[VivlioDBG][ExternalToggle] wrapper already exists in checkAndAttach');
                setWrapperEl(wrapper);
              }
            }
          } else {
        ...
        // eslint-disable-next-line no-console
        console.debug('[VivlioDBG][ExternalToggle] not editing, calling detachIfAttached');
        detachIfAttached();
      }
    };

...
export function useEditorMarkdown(opts: Options = {}) {
  ...
  const emit = React.useCallback((raw: string) => {
    ...
    // Check if CSS block changed (for diagnostics)
    const hasCssBlockBefore = /```vivliostylecss/i.test(before);
    const hasCssBlockAfter = /```vivliostylecss/i.test(raw);
    const cssBlockChanged = hasCssBlockBefore !== hasCssBlockAfter || (hasCssBlockBefore && hasCssBlockAfter && before !== raw);
          ...
          try {
            ...
            if (EditorViewCtor && EditorViewCtor.updateListener && typeof EditorViewCtor.updateListener.of === 'function') {
              ...
              const listener = EditorViewCtor.updateListener.of((u: any) => {
                try {
                  // eslint-disable-next-line no-console
                  console.debug('[VivlioDBG] useEditorMarkdown: CM6 updateListener fired', { docChanged: !!u.docChanged, updateType: u.docChanged ? 'DOC_CHANGED' : 'OTHER' });
              ...
              if (!appended) {
                // removed editor-input polling: attach immediate input listeners where possible
                ...
                try {
                  ...
                  if (host) {
                    ...
                    if (textarea) {
                      ...
                      domHandlersRef.current.push({ target: textarea, event: 'input', fn: handler });
                      // eslint-disable-next-line no-console
                      console.debug('[VivlioDBG] useEditorMarkdown: attached input listener to hidden textarea');
                    } else {
                ...
                try {
                  ...
                  const mutationCb = (muts: MutationRecord[]) => {
                    ...
                    let reason = '';
                    for (const m of muts) {
                      if (m.addedNodes && m.addedNodes.length > 0) { seen = true; reason = 'addedNodes'; break; }
                      if (m.type === 'characterData') { seen = true; reason = 'characterData'; break; }
                      if (m.type === 'attributes') { seen = true; reason = 'attributes'; break; }
                      const tgt = m.target as HTMLElement | null;
                      if (tgt && viewerSelectors.some(s => tgt.matches?.(s) || tgt.closest?.(s))) { seen = true; reason = 'viewerUpdate'; break; }
                    }
                    if (seen) {
                      // eslint-disable-next-line no-console
                      console.debug('[VivlioDBG] useEditorMarkdown: MutationObserver triggered', { reason });

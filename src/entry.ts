...
function attachEditorListeners(): boolean {
  const disposers: Array<() => void> = [];

  // 1) CodeMirror v5 instance via DOM
  try {
    const cmHost = document.querySelector('.CodeMirror') as any;
  ...
  try {
    ...
    if (EditorView && cm6root && typeof EditorView.findFromDOM === 'function') {
      ...
      if (view && view.state) {
        const read = () => { try {
          const txt = view.state.doc && typeof view.state.doc.toString === 'function'
            ? view.state.doc.toString()
            : (typeof view.state.sliceDoc === 'function' ? view.state.sliceDoc() : '');
          scheduleRender(txt);
        } catch (e) { /* ignore */ } };
        read();
        try {
  ...
  try {
    const ta = document.querySelector('textarea[name="markdown"], textarea.markdown-body, textarea') as HTMLTextAreaElement | null;
    if (ta) {
      const handler = () => { scheduleRender((ta as HTMLTextAreaElement).value); };
      ta.addEventListener('input', handler);
      disposers.push(() => ta.removeEventListener('input', handler));
      scheduleRender((ta as HTMLTextAreaElement).value);
    }
  } catch (e) { /* ignore */ }

  if (disposers.length) {
    detachFns.push(...disposers);
    console.info('[VIVLIO_DEV] editor listeners attached');
    return true;
  }
  return false;
}
...
function activate(){
  ...
  // Injection is now handled by React component BuildPdfMenuInjector mounted via client-entry
}
...
// Injection is handled by React component BuildPdfMenuInjector mounted via client-entry
